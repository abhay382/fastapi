...
class UserCreate(BaseModel):
    ...
    def validate_role(cls, value):
        if value not in ["admin", "client", "user"]:
            raise ValueError("Role must be 'admin', 'client', or 'user'")
        return value
...
class UserUpdate(BaseModel):
    ...
    @validator("role")
    def validate_role(cls, value):
        if value is not None and value not in ["admin", "client", "user"]:
            raise ValueError("Role must be 'admin', 'client', or 'user'")
        return value
...
async def upload_profile_picture(
    ...
    filename = f"{current_user.id}_{secrets.token_hex(16)}.{file_extension}"
...
# Admin endpoints
@app.post("/admin/users", response_model=TokenResponse)
...
async def admin_update_user(
    ...
    if user_update.email:
        ...
        await send_verification_email(user, background_tasks, db)
    if user_update.role:
